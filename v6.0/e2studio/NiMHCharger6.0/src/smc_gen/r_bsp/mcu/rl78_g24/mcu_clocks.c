...
*                               Changed the error check of the set_fclk_clock_source function.
*                               When HIOCLK is set as an argument of the change_clock_setting function,
*                               the process has been changed so that the register setting order is selected
*                               according to the setting contents.
*                               Removed the process not to set FLMODE and PFBE when PLLCLK is set
*                               as an argument of the change_clock_setting function.
*         : 30.11.2023 1.62     Added processing to check CLS after setting CSS bit.
*                               Added processing to check CLSTR after setting CKSELR bit.
...
*                               Added spaces after equal operator.
***********************************************************************************************************************/
/*************************************************
 ...
 *                BSP_ERROR1    Setting prohibited the PLL operating when CPU is supplied the subsystem clock.
 *                              Setting prohibited the PLL operating when FRQSEL4 = 1.
        ...
        case PLLCLK:
            ...
            if (0U == DSCON)
            {
                ...
                else
                {
                    ...
                    /* WAIT_LOOP */
                    for (w_count = 0U; w_count <= BSP_CFG_LOCKUP_WAITTIME; w_count++)
                    {
                        BSP_NOP();
                    }
                }
    ...
    if (mode != old_mode)
    {
        switch (mode)
        {
            case HIOCLK:
                ...
                else if ((PLLCLK == old_mode) && (0U != MCS))
                {
                    /* Error if the high-speed on-chip oscillator is not selected. */
                    status = BSP_ERROR3;
                }
                ...
                else
                {
                    ...
                    /* WAIT_LOOP */
                    /* Confirm that on-chip oscillator clock(fOCO)/high-speed system clock(fMX) is selected. */
                    while (1U == CLSTR)
                    {
                        ;
                    }
            ...
            case PLLCLK:
                ...
                if (0U == DSCON)
                {
                    /* Error if the PLL clock is stopped. */
                    status = BSP_ERROR1;
                }
#if BSP_CFG_PARAM_CHECKING_ENABLE == 1
                else if (1U == CLS)
                ...
                {
                    /* Error if the PLL clock is not suupplied. */
                    status = BSP_ERROR1;
                }
#endif
                else
                {
                    ...
                    /* WAIT_LOOP */
                    /* Confirm that PLL frequency-divided clock(fPLLDIV) is selected. */
                    while (0U == CLSTR)
                    {
                        ;
                    }
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                sys_clock_src_freq = g_fih_hz[((temp1 & 0x01U) << 5U) | (temp2 & 0x18U) | (temp3 & 0x07U)];
            }
        }
        
        /* fMAIN clock source is fPLL */
        /* fMAIN  : Main system/PLL select clock */
        /* fPLL : PLL clock */
        if ((1U == PLLCK) && (1U == DSCON) && (1U == CLSTR) && (0U == MCS1))
        {
            /* PLL multiplication selection */
            if (0x4FU == DSCCTL)
            {
                /* Multiples the clock frequency by 8 */
                sys_clock_src_freq *= 8U;
            }
            else if (0x57U == DSCCTL)
            {
                /* Multiples the clock frequency by 12 */
                sys_clock_src_freq *= 12U;
            }
            else
            {
                /* Setting prohibited */
...
e_bsp_err_t change_clock_setting(e_clock_mode_t mode, uint8_t * set_values)
{
    ...
    switch (mode)
    {
        case HIOCLK:
...
#if BSP_CFG_PARAM_CHECKING_ENABLE == 1
            else if ((0U != MCS1) || (0U != MCS) || (0U != CLS) || (0U != CLSTR))
            {
                status = BSP_ERROR1;
            }
#endif
            ...
            {
                /* When changing the upper limit frequency of the flash operation mode to a higher speed, */
                /* change the flash operation mode before changing the frequency. */
                if (((set_values[0] + 1) << 6) >= FLMODE)
                {
                    FLMWEN = 1U;
                    ...
                    /* WAIT_LOOP */
                    for (w_count = 0U; w_count <= 40U; w_count++) /* 225 cycle or more. */
                    {
                        BSP_NOP();
                    }
                ...
                /* When changing the flash operation mode to HS(high-speed main)mode, */
                ...
                /* When changing FIHSEL to 1, set FIHSEL after setting HOCODIV. */
                if (1U == set_values[3])
                {
                    HOCODIV = set_values[1];

                    FIHSEL = set_values[3];

                    if (0x10U == (OPTBYTE_C2 & 0x10U))
                    {
                        while (FIHST == set_values[3])
                        {
                            ;
                        }
                    }
                ...
                /* When changing FIHSEL to 0, set HOCODIV after setting FIHSEL. */
                else
                {
                    FIHSEL = set_values[3];
                ...
                /* When changing the flash operation mode to other than HS(high-speed main)mode, */
                ...
                /* When changing the upper limit frequency of the flash operation mode to a lower speed, */
                /* change the frequency first and then change the flash operation mode. */
                if (((set_values[0] + 1) << 6) < FLMODE)
                {
                    FLMWEN = 1U;
...
{
    uint8_t  cmc_tmp;
#if (BSP_CFG_MOCO_OPERATION == 1) || ((BSP_CFG_SUBCLK_OPERATION == 0) && \
    (BSP_CFG_SUBCLK_SOURCE == 1)) || BSP_CFG_SUBSYSCLK_SOURCE == 1 || \
    (BSP_CFG_PLLCLK_OPERATION == 1)
    volatile uint32_t w_count;
#endif
    ...
    FLMWEN = 1U;
#if BSP_CFG_FLASH_MODE == 0
    FLMODE = 0x40U;
#elif BSP_CFG_FLASH_MODE == 1
    FLMODE = 0x80U;
#else
    FLMODE = 0xC0U;
#endif
    ...
    PFBE    = BSP_CFG_PFB_ENABLE;
    FIHSEL  = BSP_CFG_FIH_SEL;

    if (0x10U == (OPTBYTE_C2 & 0x10U))
    {
        while (BSP_CFG_FIH_SEL == FIHST)
        {
            ;
        }
    }
...
#if (BSP_CFG_HISYSCLK_SOURCE != 0) && (BSP_CFG_HISYSCLK_OPERATION == 0)
    /* WAIT_LOOP */
    /* Confirm that high-speed system clock(fMX) is selected. */
    while (0U == MCS)
    {
...
#if (BSP_CFG_SUBSYSCLK_SOURCE == 1) || ((BSP_CFG_SUBSYSCLK_SOURCE == 0) && (BSP_CFG_SUBCLK_SOURCE != 0) && (BSP_CFG_SUBCLK_OPERATION == 0))
    /* WAIT_LOOP */
    /* Confirm that subsystem clock(fSUB) is selected. */
    while (0U == CLS)
    {
...
#if BSP_CFG_PLLCLK_SOURCE == 1
    PLLCK = 1U;
#endif /* BSP_CFG_PLLCLK_SOURCE == 1 */
    DSCON = 1U;
...
#endif /* BSP_CFG_PLLCLK_OPERATION == 1 */
